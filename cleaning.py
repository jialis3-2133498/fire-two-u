...
# ── 2. DUPLICATE ROWS ─────────────────────────────────────────────────────────
print(f"\nDuplicate rows — train: {train.duplicated().sum()}, test: {test.duplicated().sum()}")
...
# Note: for events that never hit (event=0), time_to_hit_hours is the censoring time
