...
def train_random_forest_model(
    ...
    model = RandomForestClassifier(
        ...
        class_weight="balanced",  # handles class imbalance (~31% positive rate)
...
def train_all_random_forest_models(
    ...
    for each_target_col in target_cols:
        model, imputer, auc = train_random_forest_model(
            train_df, valid_df, feature_cols, each_target_col
        )
        model_dict[each_target_col] = {"model": model, "imputer": imputer, "auc": auc}
...
def predict_all_horizons_rf(
    ...
    for each_key in fitted_models.keys():
        ...
        y_prob = predict_random_forest_model(each_model, each_imputer, df, feature_cols)
...
# It sorts by y_24 by default since that's usually the most informative middle horizon.
def get_feature_importances(
    ...
    return pd.DataFrame(rows, index=feature_cols).sort_values("y_24", ascending=False)
